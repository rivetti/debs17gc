...
public class PendingCircularBuffer2<T extends Tuple> implements Iterable<T>, Iterator<T> {

	private final Logger LOGGER = LoggerFactory.getLogger(PendingCircularBuffer2.class);
	...
	public PendingCircularBuffer2(int size, long tsStep) {
		this.tsStep = tsStep;
	...
	public Tuple get(long ts) {
		if (ts != smallestTS) {
			throw new RuntimeException();
		}
		return buffer[smallestTSPointer];
	}

	public Tuple remove(long ts) {
		if (ts != smallestTS) {
			throw new RuntimeException();
		}
	...
	public void increase(int offsetPosition) {
		...
		LOGGER.debug("Increase from {} to {}", targetSize, newSize);
	...
	public static void main(String[] args) {
		PendingCircularBuffer2<Tuple1<Integer>> buf = new PendingCircularBuffer2<>(1, 10);
		buf.setInitialTS(20);
